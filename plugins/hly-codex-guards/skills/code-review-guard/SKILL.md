...
## Logging Review Guard

当本次 Diff 涉及日志新增、日志删除、日志文案调整、日志级别调整，或关键链路缺失日志时，必须检查日志是否真正服务于线上排查和业务定位。

日志 Review 只输出会影响排查、合规、安全或运维定位的真实风险；不要输出单纯日志风格建议。
...
- 是否存在循环内逐条打印日志。
- 是否存在无意义的入口、出口日志。
- 是否在 Getter / Setter / DTO / VO 转换 / Bean 复制 / MapStruct 转换 / DAO 普通 CRUD / 工具类中新增无效日志。
- 是否使用 `DEBUG`、`TRACE`、`WARN`。
- `WARN` 场景是否统一使用 `INFO` 并增加 `[WARNING]` 标识。
- 是否用字符串拼接构造日志内容，而不是使用占位符。
- 是否泄露 `password`、`token`、`accessToken`、`refreshToken`、`sign`、`secret`、身份证号、银行卡号、完整手机号、完整邮箱等敏感信息。
- 非日志需求中是否顺手修改日志级别、日志文案、日志结构或删除日志。

### 日志风险判断

以下情况应标记为风险：

- 外部接口调用、MQ 生产、MQ 消费、Job 执行、业务状态流转、数据修复、关键兜底或降级逻辑缺少必要日志，导致线上问题难以定位。
- `ERROR` 日志只打印异常消息，没有打印完整异常堆栈。
- 日志缺少 `orderNo`、`supplierOrderNo`、`tenantId`、`employeeOid`、`companyOid`、`journeyNo`、`approvalNo`、`batchNo` 等必要业务主键，导致无法定位具体数据。
- 循环内逐条打印日志，可能造成日志量失控。
- 无意义的入口、出口日志挤占有效日志。
- 日志打印敏感信息，存在安全或合规风险。
- 非日志需求中顺手改日志，且没有说明原因、排查价值和影响范围。
...
5. 是否存在敏感信息泄露风险。
6. 日志变更是否属于本次需求必要改动。
7. 如果建议新增日志，必须说明用于排查什么问题、对应哪个业务链路。

如果无法确认日志价值或业务链路，必须标记为“不确定”，不能猜测。
